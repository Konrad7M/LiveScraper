...
def parseTwitch(url):
        ...
        #url = sys.argv[1]
        #url = r'https://www.twitch.tv/polsatgames'
        ...
        while True: #timeout > time.time():
                ...
                chats = list(browser.find_elements_by_css_selector('chat-line__no-background tw-inline'))
                ...
                if bool(chats_read):
                        for chat in chats_read:
                                ...
                                author_name = chat.find_element_by_css_selector("#chat-line__display-name").get_attribute('innerText')
                                message = chat.find_element_by_css_selector("#text-fragment").get_attribute('innerText')
                                print("[" + author_name + "|"+ time.strftime(' %H:%M:%S ') + "]:" + message + "\n")
                chats_prev = chats
        browser.quit()
        return chats
